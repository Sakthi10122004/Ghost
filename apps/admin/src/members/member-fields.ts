...
import {FILTER_TYPES, type FieldDescriptor, buildCatalog, domainField, columnAddressing} from '@/shared/filters';
...
const MEMBER_FIELDS = [
    {key: 'name', icon: 'person', type: 'text', ui: {label: 'Name', placeholder: 'Enter name...', className: 'w-48'}},
    {key: 'email', icon: 'mail', type: 'text', ui: {label: 'Email', placeholder: 'Enter email...', className: 'w-48'}},
    {key: 'label', icon: 'tag', type: 'set', ui: {label: 'Label', searchable: true, className: 'w-64'},
        metadata: {activeColumn: {key: 'labels', label: 'Labels'}, columnInclude: 'labels'}},
    domainField({key: 'subscribed', icon: 'mail', semantics: subscriptionSemantics(), operators: FILTER_TYPES.scalar.operators,
    ...
    {key: 'last_seen_at', icon: 'eye', type: 'timestamp', relative: 'past', ui: {label: 'Last seen'}},
    {key: 'created_at', icon: 'calendar', type: 'timestamp', relative: 'past', ui: {label: 'Created'}},
    {key: 'signup', icon: 'person-plus', type: 'scalar', valueConfig: {quoteStrings: true},
        ui: {label: 'Signed up on post/page', searchable: true, placeholder: 'Select a post or page...', className: 'w-64'}},
    {key: 'tier_id', icon: 'card', type: 'set', ui: {label: 'Membership tier', searchable: true, className: 'w-64'},
        metadata: {activeColumn: {key: 'tiers', label: 'Tiers'}, columnInclude: 'tiers'}},
    {key: 'status', icon: 'person-circle', type: 'scalar', ui: {label: 'Member status', searchable: false},
        options: [
            {value: 'paid', label: 'Paid'},
            {value: 'free', label: 'Free'},
            {value: 'comped', label: 'Complimentary'}
        ]},
    {key: 'subscriptions.plan_interval', icon: 'calendar-clock', type: 'scalar', ui: {label: 'Billing period', searchable: false},
        options: [
            {value: 'month', label: 'Monthly'},
            {value: 'year', label: 'Yearly'}
        ],
        metadata: {activeColumn: {key: 'subscriptions.plan_interval', label: 'Billing period'}, columnInclude: 'subscriptions'}},
    {key: 'subscriptions.status', icon: 'card', type: 'scalar', ui: {label: 'Stripe subscription status', searchable: false},
        options: SUBSCRIPTION_STATUS_OPTIONS,
        metadata: {activeColumn: {key: 'subscriptions.status', label: 'Subscription status'}, columnInclude: 'subscriptions'}},
    {key: 'subscriptions.start_date', icon: 'calendar-start', type: 'timestamp', relative: 'past', ui: {label: 'Paid start date'},
        metadata: {activeColumn: {key: 'subscriptions.start_date', label: 'Paid start date'}, columnInclude: 'subscriptions'}},
    {key: 'subscriptions.current_period_end', icon: 'calendar-end', type: 'timestamp', relative: 'future', ui: {label: 'Next billing date'},
        metadata: {activeColumn: {key: 'subscriptions.current_period_end', label: 'Next billing date'}, columnInclude: 'subscriptions'}},
    {key: 'conversion', icon: 'arrows', type: 'scalar', valueConfig: {quoteStrings: true},
        ui: {label: 'Subscription started on post/page', searchable: true, placeholder: 'Select a post or page...', className: 'w-64'}},
    {key: 'email_count', icon: 'send', type: 'number', ui: {label: 'Emails sent (all time)', defaultOperator: 'is-greater', min: 0, className: 'w-24'}},
    {key: 'email_opened_count', icon: 'mail-open', type: 'number', ui: {label: 'Emails opened (all time)', defaultOperator: 'is-greater', min: 0, className: 'w-24'}},
    {key: 'email_open_rate', icon: 'percent', type: 'number', ui: {label: 'Open rate (all time)', defaultOperator: 'is-greater', min: 0, max: 100, suffix: '%', className: 'w-24'}},
    {key: 'emails.post_id', icon: 'send', type: 'scalar', valueConfig: {quoteStrings: true},
        ui: {label: 'Sent email', searchable: true, placeholder: 'Select an email...', className: 'w-64'}},
    {key: 'opened_emails.post_id', icon: 'mail-open', type: 'scalar', valueConfig: {quoteStrings: true},
        ui: {label: 'Opened email', searchable: true, placeholder: 'Select an email...', className: 'w-64'}},
    {key: 'clicked_links.post_id', icon: 'click', type: 'scalar', valueConfig: {quoteStrings: true},
        ui: {label: 'Clicked email', searchable: true, placeholder: 'Select an email...', className: 'w-64'}},
    domainField({key: 'newsletter_feedback', icon: 'message', semantics: feedbackSemantics(), addressing: columnAddressing({field: 'feedback.post_id'}), operators: ['1', '0'],
    ...
    {key: 'offer_redemptions', icon: 'ticket', type: 'set', valueConfig: {quoteStrings: true, serializeSingletonAsScalar: true},
        ui: {label: 'Offer', searchable: true, className: 'w-64'},
        metadata: {activeColumn: {key: 'offer_redemptions', label: 'Offer'}}},
    {key: MULTIPLE_ACTIVE_STRIPE_CUSTOMERS_FIELD, icon: 'layers', type: 'count', valueConfig: {threshold: 1, absentForm: 'below'},
...
export const CUSTOM_FIELD_OPERATORS = [...FILTER_TYPES.text.operators, ...CUSTOM_FIELD_SET_OPERATORS];
...
const CUSTOM_FIELD: FieldDescriptor = {
    key: 'custom_fields.:key', icon: 'text',
    type: 'text',
