...
import {RELATIVE_FUTURE_OPERATOR, RELATIVE_PAST_OPERATOR} from './filter-relative-date';
...
type RelativeDirection<TType extends FilterTypeId> =
    | ('in-the-last' extends WritableBy<TType> ? 'past' : never)
    | ('in-the-next' extends WritableBy<TType> ? 'future' : never);

type TypeSpecific<TType extends FilterTypeId> = {
    type: TType;
    valueConfig?: ConfigOf<TType>;
    relative?: RelativeDirection<TType>;
};
...
export function describeField(descriptor: FieldDescriptor): FilterField {
    ...
    // A type offers a sensible default set, and a field can ask for the relative operator on top
    // of it. Applied here so it works for any resource that declares it, rather than depending on
    // each one remembering to add it.
    const relative: readonly string[] = descriptor.relative
        ? [descriptor.relative === 'past' ? RELATIVE_PAST_OPERATOR : RELATIVE_FUTURE_OPERATOR]
        : [];
    const offered = [...(registered?.operators ?? semantics.operators), ...presenceOperators, ...relative];
    ...
    return {
        operators: (descriptor.operators?.filter(operator => encodable.includes(operator)) ?? offered) satisfies readonly string[],
