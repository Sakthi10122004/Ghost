...
function namesKey(node: AstNode, keyOrNamespace: string): boolean {
    if (Object.keys(node).some(key => !key.startsWith('$') && key.startsWith(keyOrNamespace))) {
        return true;
    }
...
export function parseNodeToPredicates<TFields extends Record<string, FilterField>>(node: AstNode, fields: TFields, timezone: string): ParsedPredicate[] {
...
// cannot operate.
export function isPredicateEnabled<TFields extends Record<string, FilterField>>(predicate: ParsedPredicate, fields: TFields): boolean {
