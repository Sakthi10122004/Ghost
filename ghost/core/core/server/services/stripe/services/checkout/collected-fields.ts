...
export interface CollectedFieldsDeps {
    activeFieldKeys(): Promise<Set<string>>;
    resolvePort(port: string): Promise<{key: string} | null>;
    planWrite(values: Record<string, unknown>): Promise<unknown[]>;
    applyWrite(memberId: string, plan: unknown[]): Promise<void>;
}
...
const PORT_VALUES: Record<string, (session: CompletedSession) => unknown> = {
...
export async function collectedValues(session: CompletedSession, deps: CollectedFieldsDeps): Promise<CollectedValue[]> {
    ...
    const activeKeys = await deps.activeFieldKeys();
    for (const field of session.custom_fields ?? []) {
        const key = field?.key;
        const answer = text(field?.text?.value);
        if (key && answer && activeKeys.has(key)) {
            collected.push({key, value: answer});
        }
    }

    for (const [port, read] of Object.entries(PORT_VALUES)) {
        ...
        collected.push({key: destination.key, value});
    }
...
export async function writeCollectedFields(memberId: string, session: CompletedSession, deps: CollectedFieldsDeps): Promise<void> {
    ...
    for (const {key, value} of collected) {
        try {
            plan.push(...await deps.planWrite({[key]: value}));
        } catch (err) {
            logging.warn({
    ...
    try {
        await deps.applyWrite(memberId, plan);
    } catch (err) {
        logging.error(err);
    }
}
