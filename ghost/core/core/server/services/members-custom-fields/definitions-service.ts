...
function fold(rows: Array<DependencyRow & Record<string, unknown>>, relations: DefinitionRelation[]) {
    ...
    const seenTiers = new Map<string, Set<string>>();
    ...
    for (const row of rows) {
        ...
        if (!field) {
            ...
            seenTiers.set(row.key, new Set());
        }

        // The tiers join multiplies the rows, so a binding arrives once per tier.
        if (row.bound_port && field.bindings?.length === 0) {
            field.bindings.push({port: row.bound_port});
        }
        if (row.tier_id && row.tier_name && !seenTiers.get(row.key)!.has(row.tier_id)) {
            seenTiers.get(row.key)!.add(row.tier_id);
