...
const CHECKOUT_FIELDS_TABLE = 'products_checkout_fields';
 ...
 * back into one — a field has at most one binding, so only the tiers multiply it.
 *
...
export function withDependencies<T extends Knex.QueryBuilder>(query: T, relations: DefinitionRelation[]): T {
    ...
    if (relations.includes('tiers')) {
        query
            .leftJoin(CHECKOUT_FIELDS_TABLE, `${CHECKOUT_FIELDS_TABLE}.custom_field_key`, `${FIELDS_TABLE}.key`)
            .leftJoin(PRODUCTS_TABLE, `${PRODUCTS_TABLE}.id`, `${CHECKOUT_FIELDS_TABLE}.product_id`);
