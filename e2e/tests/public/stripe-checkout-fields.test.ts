 ...
 *
 * The completion is built from a session captured from Stripe test mode, so the payload
 * shape is Stripe's rather than one we wrote to match our reader.
 */
...
test.describe('Ghost Public - Stripe Checkout Fields', () => {
    ...
    test('collects a publisher\'s questions and a delivery address, and saves both on the member', async ({page, stripe}) => {
        ...
        // Stripe's address parts are exactly ours, so nothing is transformed. The recipient
        // name sits beside the address on Stripe's side and inside it on ours.
